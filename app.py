...
def montar_bloco_informacoes_lote(campos):
    ...
    data_final = data_visual if data_visual else "não encontrado"
    torneio_final = torneio if torneio else "não encontrado"
    cidade_final = cidade_uf if cidade_uf else "não encontrado"
    local_final = local if local else "não encontrado"
    categorias_final = categorias if categorias else "não encontrado"
    contato_final = contato if contato else "não encontrado"

    return (
        f"Data: {data_final}\n"
        f"Torneio: {torneio_final}\n"
        f"Cidade/ES: {cidade_final}\n"
        f"Local: {local_final}\n"
        f"Categorias: {categorias_final}\n"
        f"Contato: {contato_final}"
    )
...
def listar_pendencias_lote(campos):
    ...
    pendencias = [campo for campo, valor in bloco.items() if not valor or valor == "não encontrado"]
    return pendencias
...
def montar_mensagem_direct_lote(campos):
    ...
    if len(pendencias) == 1:
        titulo_falta = "Falta apenas a informação abaixo:"
    else:
        titulo_falta = "Faltam apenas as informações abaixo:"
...
def extrair_texto_1_torneio(imagens, informacao_complementar=""):
...
def extrair_texto_lote_1_torneio(imagem):
    ano_corrente_2 = str(datetime.now().year)[-2:]
...
Regras obrigatórias:
- Em Instagrams, liste TODOS os perfis de Instagram que conseguir identificar na postagem, inclusive collabs, separados por espaço.
...
"""
    ...
    conteudo = [
        ...
        {"type": "input_image", "image_url": imagem_para_data_url(imagem)}
    ]
